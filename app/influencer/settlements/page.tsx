...
export default async function InfluencerSettlementsPage() {
  ...
  const { data } = await supabase
    .from("settlements")
    .select(
      `id, gross_amount, commission_amount, payout_amount, status, created_at, paid_at,
       order_item:order_items(product_name)`,
    )
    .eq("beneficiary_id", profile.id)
    .eq("beneficiary_type", "influencer")
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">공동구매 수익 정산 내역입니다.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-sm text-zinc-500">정산 대기</CardTitle></CardHeader>
          <CardContent><p className="text-2xl font-bold text-[var(--price)]">{formatKRW(pendingSum)}</p></CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-sm text-zinc-500">정산 완료</CardTitle></CardHeader>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-zinc-100 last:border-0">
                  ...
                  <td className="px-4 py-3">{r.order_item?.product_name ?? "-"}</td>
